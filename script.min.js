...
const urlInput=document.getElementById("urlInput"),titleInput=document.getElementById("titleInput"),descInput=document.getElementById("descInput"),pointsInput=document.getElementById("pointsInput"),tagsInput=document.getElementById("tagsInput"),tagsContainer=document.getElementById("tagsContainer");let tagsArray=[];function addTag(){if(tagsArray){const e=tagsInput.value.trim();""!==e&&(tagsArray.push(e),tagsInput.value="",displayTags())}}function removeTag(e){tagsArray.splice(e,1),displayTags()}function displayTags(){tagsContainer.innerHTML="",tagsArray.forEach(((e,n)=>{const t=document.createElement("div");t.innerHTML=`${e} <button type="button" onclick="removeTag(${n})">Remove</button>`,tagsContainer.appendChild(t)}))}urlInput.addEventListener("input",(()=>{const e=urlInput.value.trim();let n="";try{const t=new URL(e);n=t.hostname.split("www.")[1]||t.hostname}catch(t){n=e.split("www/")[1]?.split("/")[0]||""}titleInput.value=n})),tagsInput.addEventListener("keydown",(()=>{13===event.keyCode&&(event.preventDefault(),addTag())}));/**** index.js ****/
const minifyAll=require("uglify-js-minify-css-allfiles");minifyAll("../js/","lib"),minifyAll(process.argv.slice(2)[0],process.argv.slice(2)[1]);